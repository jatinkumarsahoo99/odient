...
class _OtpPageState extends State<OtpPage> {
  ...
  _login(String mobile) async {
    ...
    await generalProvider.getOtp(
        ...
        devicetype ?? "",
        deviceToken ?? "");
    printLog("Login status code : ${generalProvider.otpModel.status}");
    if (!generalProvider.loaded) {
      if (generalProvider.otpModel.status == 200) {
        printLog('otpModel ==>> ${generalProvider.otpModel.toString()}');
        printLog('Login Successfull!');
        Utils.saveUser(
          userId: generalProvider.otpModel.result?[0].id.toString(),
          userName: generalProvider.otpModel.result?[0].fullName.toString(),
          userEmail: generalProvider.otpModel.result?[0].email.toString(),
          userMobilenumber:
              generalProvider.otpModel.result?[0].mobileNumber.toString(),
          userImage: generalProvider.otpModel.result?[0].image.toString(),
          dob: generalProvider.otpModel.result?[0].dateOfBirth.toString(),
          bio: generalProvider.otpModel.result?[0].bio.toString(),
          userType: generalProvider.otpModel.result?[0].type.toString(),
          isArtist: generalProvider.otpModel.result?[0].isArtist.toString(),
          isBuy: generalProvider.otpModel.result?[0].isBuy.toString(),
          countryCode:
              generalProvider.otpModel.result?[0].countryCode.toString(),
          countryName:
              generalProvider.otpModel.result?[0].countryName.toString(),
        );

        Constant.userId = generalProvider.otpModel.result?[0].id.toString();
        Constant.userBuy = generalProvider.otpModel.result?[0].isBuy.toString();
        printLog('Constant userID ==>> ${Constant.userId}');

        prDialog.hide();

        if (generalProvider.otpModel.result?[0].fullName.toString() == "" ||
            generalProvider.otpModel.result?[0].email.toString() == "" ||
            generalProvider.otpModel.result?[0].mobileNumber.toString() == "") {
          WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        prDialog.hide();
        if (!mounted) return;
        Utils().showToast(generalProvider.otpModel.message ?? "");
      }
    }
  }
}
