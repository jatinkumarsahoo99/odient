...
class MakePaymentStage extends StatefulWidget {
  final String? id, fees, requestId, description;
  ...
  @override
  State<MakePaymentStage> createState() => _MakePaymentStageState();
}

class _MakePaymentStageState extends State<MakePaymentStage> {
  String? username, useremail, userMobilenumber, paymentId, status;
  ...
  Future<void> getApi() async {
    ...
    await paymentOptionProvider.setFinalAmount(widget.fees);
  ...
  Future<void> getPaymentTran() async {
    ...
    await paymentOptionProvider.getUserTransacation(
        widget.id ?? "",
        widget.requestId ?? "",
        widget.fees ?? "",
        paymentId ?? "",
  ...
  void handlePaymentErrorResponse(PaymentFailureResponse response) async {
    ...
    * */
    status = "0";
    printLog("Transcation Status is : $status");
  ...
  _flutterwaveInit() async {
    if (paymentOptionProvider.paymentOptionModel.result?.flutterwave != null) {
      ...
      final Flutterwave flutterwave = Flutterwave(
        ...
        txRef: const Uuid().v1(),
        amount: widget.fees.toString().trim(),
        customer: customer,
